...
void	create_res_cam(t_scene *sc, char *data, char *data_type, int *i)
{
	...
	{
		has_n_param(sc, data, 8);
		sc->res.width = (int)fmin(1920, fmax(1, ft_atof(data, i)));
		sc->res.height = (int)fmin(1080, fmax(1, ft_atof(data, i)));
		sc->antialiasing = (int)fmin(1, fmax(0, ft_atof(data, i)));
		sc->diffuse_light = (int)fmin(1, fmax(0, ft_atof(data, i)));
		sc->nrays = (int)fmin(100000, fmax(0, ft_atof(data, i)));
		sc->nb_rebound = (int)fmin(100000, fmax(3, ft_atof(data, i)));
		sc->sepia = (int)fmin(1, fmax(0, ft_atof(data, i)));
	}
	else
	{
